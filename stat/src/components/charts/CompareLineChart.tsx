...
interface CompareLineChartProps {
    ...
    selectedColors: { [blockId: string]: string };
    selectedOpacity: { [blockId: string]: number };
}

export function CompareLineChart({selectedFilters, selectedColors, selectedOpacity}: CompareLineChartProps) {
    ...
    useEffect(() => {
        ...
        const xAxis = axisBottom(xScale).ticks(10).tickFormat(d => `${d}`);
        const yAxis = axisLeft(yScale).ticks(10).tickFormat(d => `${d}`);
        ...
        dataToPlot.forEach(({ blockId, data, columns }, blockIndex) => {
            columns.forEach((column: string, colIndex: number) => {
                ...
                svg.append("path")
                    .datum(lineData)
                    .attr("fill", "none")
                    .attr("stroke", selectedColors[blockId] || COLOR_PALETTE[blockIndex % COLOR_PALETTE.length]) // Use block color
                    .attr("stroke-width", 1.5)
                    .attr("transform", `translate(${margin.left}, ${margin.top})`)
                    .attr("d", lineGenerator as any)
                    .style("stroke-dasharray", columnPatterns[column])
                    .attr("stroke-opacity", blockOpacity / 100)
                    .on("click", () => setSelectedLineIndex(selectedLineIndex === lineIndex ? null : lineIndex));
            });
        });
    }, [dataToPlot, selectedLineIndex, selectedColors, selectedOpacity]);
