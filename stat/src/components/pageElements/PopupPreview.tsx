...
const StyledPopup = styled.div<{ $large: boolean}>`
  ...
  align-items: center;
  width: ${props => props.$large ? '1200px' : '600px'};
  height: ${props => props.$large ? '800px' : '400px'};
...
export function PopupPreview({ children, title, showPopup, large=false}: { children: ReactNode, title: string, showPopup: (arg0: boolean) => void, large?: boolean }) {
    return (
